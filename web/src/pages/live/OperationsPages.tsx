...
export function InfrastructurePage() {
  ...
  const canReadRelays = Boolean(network && hasPermission('relay.read', network.network_id))
  ...
  return <><PageHeader title="Infrastructure" action={network && hasPermission('relay.manage', network.network_id) ? <Button to="/infrastructure/relays/new" variant="primary"><RadioTower size={16} />Register relay</Button> : undefined} /><SummaryStrip label="Infrastructure summary" items={[{ label: 'Networks', value: inventory?.networks.length ?? 0 }, { label: 'IPv4 space', value: network?.ipv4_pool ?? 'Not set', detail: network?.name ?? 'No network' }, { label: 'Enabled relays', value: canReadRelays ? records.filter((relay) => relay.enabled).length : '—', detail: canReadRelays ? `${records.length} registered` : 'Not authorized' }, { label: 'Runtime status', value: 'Not reported', detail: 'No relay health telemetry' }]} /><div className="infrastructure-spotlight"><div><h2>{network?.name ?? 'No selected network'}</h2><p>{network ? `${network.ipv4_pool}${network.ipv6_pool ? ` · ${network.ipv6_pool}` : ''}` : 'No network'}</p></div><Button to="/networks" variant="secondary">Networks <ArrowRight size={14} /></Button></div>{canReadRelays ? <VisibilityToolbar value={visibility} onChange={setVisibility} currentLabel="Enabled relays only" visible={visibleRecords.length} total={records.length} /> : null}<div className="infrastructure-grid"><Section title="Network" meta="Current">{network ? <ResourceLink to={`/infrastructure/networks/${network.network_id}`} icon={<Network size={19} />} title={network.name} meta={`Epoch ${network.configuration_epoch} · ${network.ipv4_pool}`} state={<Status tone="muted">Configured</Status>} /> : <div className="inline-empty">No network.</div>}</Section><Section title="Relays" meta={canReadRelays && records.length ? `${records.length} registered` : undefined}>{!canReadRelays ? <DashboardEmpty icon={<RadioTower size={18} />} title="Relay inventory unavailable" description="This administrator cannot read relay records." /> : visibleRecords.length ? <div className="resource-list">{visibleRecords.map((relay) => <ResourceLink key={relay.relay_id} to={`/infrastructure/relays/${relay.relay_id}`} icon={<RadioTower size={19} />} title={relay.name} meta={relay.endpoint} state={<Status tone={relay.enabled ? 'positive' : 'muted'}>{relay.enabled ? 'Enabled' : 'Disabled'}</Status>} />)}</div> : <DashboardEmpty icon={<RadioTower size={18} />} title="No enabled relays" />}</Section></div></>
}
...
export function SecurityPage() {
  ...
  const posture = !records.length ? { label: 'No records', tone: 'muted' as const } : validRecords.length === records.length ? { label: 'Valid', tone: 'positive' as const } : { label: 'Review', tone: 'warning' as const }
