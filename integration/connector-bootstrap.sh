...
key=$(printf '%s\n' "$setup_token" | docker run --rm -i --network none \
  --user 65532:65532 --cap-drop ALL --security-opt no-new-privileges:true \
  --volume "$work_dir:/work" --entrypoint /usr/local/bin/laneway "$image" \
  connector bootstrap-seal --out /work/envelope --expires-at "$expires_at")
